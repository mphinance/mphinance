...
LOG_FILE = os.path.join(BASE_DIR, "landing/blog/blog_entries.json")
...
def parse_meetings(html_content):
    ...
    for table in tables:
        try:
            ...
            if map_link:
                # We build our own maps link to ensure search-based accuracy if needed, 
                # but we'll use theirs if available
...
def generate_html_card(m):
    ...
    # Map codes to tags
    tags = []
    # Auto-tag based on name/notes if codes are sparse
    ...
    for tag in tags:
        ...
        tag_html += f'<span class="tag tag-{tag}">{label}</span>'
        ...
        <div class="meeting-card" data-day="{day_slug}" data-types="{type_attr}" data-search="{search_text}">
            <div class="meeting-header">
                ...
                <div class="meeting-time">
                    ...
                    <span class="time-value">{m['time']}</span>
                </div>
            </div>
            <div class="meeting-details">
                <div class="detail-item">
                    <span class="detail-label">Location</span>
                    <p class="detail-value">{address_html}</p>
                </div>
                {"<div class='detail-item'><span class='detail-label'>Notes</span><p class='detail-value'>" + m['notes'] + "</p></div>" if m['notes'] else ""}
                {"<div class='detail-item'><span class='detail-label'>Additional Info</span><p class='detail-value info-box'>" + m['additional_info'] + "</p></div>" if m['additional_info'] else ""}
            </div>
            <div class="meeting-footer">
                <div class="tags-container">
                    {tag_html}
                </div>
                <a href="{m['google_maps_link']}" target="_blank" class="map-link">
