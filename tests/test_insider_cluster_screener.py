"""Tests for dossier.insider_cluster_screener pure parsing + scoring helpers."""
...
from dossier.insider_cluster_screener import (
    _is_purchase,
    _is_sale,
    _parse_date,
...
class TestTransactionClassification:
    ...
    def test_purchase_with_price_text(self):
        assert _is_purchase("Purchase at price 12.34 - 5,000 shares") is True

    def test_buy_prefix(self):
        assert _is_purchase("Buy") is True
...
def _text_row(insider, text, start_date, value=100_000):
    ...
    the actual wording ('Purchase at price X per share.') lives in Text."""
    return {"Insider": insider, "Transaction": "", "Text": text, "Start Date": start_date, "Value": value}


class TestParseInsiderTransactions:
    def test_none_df(self):
        result = _parse_insider_transactions(None)
        assert result["distinct_buyers"] == 0
        assert result["buyer_names"] == []
        assert result["total_value"] == 0.0
        assert result["sale_count"] == 0
        assert result["most_recent_buy"] is None

    def test_empty_df(self):
        result = _parse_insider_transactions(pd.DataFrame())
        assert result["distinct_buyers"] == 0

    def test_two_distinct_buyers_in_window(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 2, 1), 200_000),
            _row("John CFO", "Purchase", date(2026, 2, 15), 50_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 2
        assert result["buyer_names"] == ["Jane CEO", "John CFO"]
        assert result["total_value"] == 250_000
        assert result["most_recent_buy"] == "2026-02-15"

    def test_same_insider_twice_counts_once(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 2, 1), 100_000),
            _row("Jane CEO", "Purchase", date(2026, 2, 10), 100_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 1
        assert result["total_value"] == 200_000

    def test_stale_purchase_outside_window_excluded(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2025, 1, 1), 100_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 0
    ...
    def test_future_date_excluded(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 4, 1), 100_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 0

    def test_sales_counted_separately_from_buyers(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 2, 1), 100_000),
            _row("Bob Director", "Sale", date(2026, 2, 5), 50_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 1
    ...
    def test_missing_date_row_skipped(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", None, 100_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 0
    ...
    def test_non_numeric_value_treated_as_zero(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 2, 1), "not-a-number"),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 1
        assert result["total_value"] == 0.0

    def test_most_recent_buy_picks_latest(self):
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _row("Jane CEO", "Purchase", date(2026, 1, 5), 100_000),
            _row("John CFO", "Purchase", date(2026, 2, 20), 100_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["most_recent_buy"] == "2026-02-20"
    ...
    def test_real_world_shape_blank_transaction_wording_in_text(self):
        ...
        # verb lives in "Text" ("Purchase at price X per share.").
        as_of = date(2026, 3, 1)
        df = pd.DataFrame([
            _text_row("NOTO ANTHONY J.", "Purchase at price 18.06 per share.", date(2026, 2, 1), 250_787),
            _text_row("SCHUPPENHAUER ERIC", "Purchase at price 19.93 per share.", date(2026, 2, 10), 99_650),
            _text_row("SOME EXEC", "Sale at price 20.00 per share.", date(2026, 2, 12), 40_000),
        ])
        result = _parse_insider_transactions(df, lookback_days=90, as_of=as_of)
        assert result["distinct_buyers"] == 2
...
class TestCleanSignalScore:
    ...
    def test_two_or_more_sales(self):
...
class TestTrendContextScore:
    ...
    def test_at_or_below_sma(self):
        assert trend_context_score(95.0, 100.0) == 8

    def test_boundary_5pct_extended(self):
        assert trend_context_score(105.0, 100.0) == 8

    def test_moderately_extended(self):
        assert trend_context_score(110.0, 100.0) == 4

    def test_boundary_15pct_extended(self):
        assert trend_context_score(115.0, 100.0) == 4

    def test_very_extended(self):
        assert trend_context_score(130.0, 100.0) == 0
