...
atr_pct = {t: atr[t] / entry_price[t] for t in tickers}  # ATR as % of price -- the actual fix
CAPITAL = 100_000.0
N = len(tickers)
...
# ---------- v3 FIX: weight by ATR-as-%-of-price, not raw dollar ATR ----------
stop_dist_pct = {t: 2 * atr_pct[t] for t in tickers}
inv_stop_pct = {t: 1.0 / stop_dist_pct[t] for t in tickers}
tot_inv_stop_pct = sum(inv_stop_pct.values())
risk_weight = {t: inv_stop_pct[t] / tot_inv_stop_pct for t in tickers}
...
vol_weight = {t: inv_atr_pct[t] / tot_inv_atr_pct for t in tickers}
...
# confirm risk_weight == vol_weight (same finding as v2, constant factor of 2 cancels)
max_weight_diff = max(abs(risk_weight[t] - vol_weight[t]) for t in tickers)
...
results["risk_static"] = run_static(shares_risk, cash_risk, "Percent-risk static (v3, %ATR)")
results["vol_static"] = run_static(shares_vol, cash_vol, "Percent-volatility static (v3, %ATR)")
...
results["vol_rebal"] = run_rebalanced(vol_weight, "Percent-vol + quarterly rebalance (v3, %ATR)")
...
results["vol_topup"] = run_topups(shares_vol, cash_vol, vol_weight, "Percent-vol + monthly top-ups (v3, %ATR)")
...
print(f"max |risk_weight - vol_weight| across all tickers: {max_weight_diff:.8f} (confirms still identical under %ATR)")
print("\n=== v3 CORRECTED RESULTS (%ATR weighting) ===")
for k, r in results.items():
    print(k, r)

with open("/home/mph/mphinance/docs/drafts/tharp-port/results_summary_v3.json", "w") as f:
    json.dump(results, f, indent=2, default=str)
print("\nSaved to results_summary_v3.json")
