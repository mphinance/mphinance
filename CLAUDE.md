...
- Deploy landing: `rsync -avz landing/ vultr:/home/mphinance/public_html/`
- Deploy docs/widgets: `git push` → auto-deploys via GitHub Pages
- Tickers: `docs/ticker/` — deep dive pages, grouped by sector on index

### Alpha.HUD Widget System

Self-contained HTML widgets that pull from `docs/ticker/{TICKER}/latest.json`.

**Files:**

| File | What | Embed Size |
|------|------|-----------|
| `docs/widgets/full-hud.html` | ⭐ All-in-one combined widget | 700×760 |
| `docs/widgets/signal-bar.html` | Trend + RSI + IV + EMA stack | 700×160 |
| `docs/widgets/technical-matrix.html` | MAs, pivots, Fibonacci, oscillators | 700×540 |
| `docs/widgets/ai-synthesis.html` | AI grade + tech/fund scores + analysis | 420×400 |
| `docs/widgets/valuation.html` | Graham+Lynch+Analyst blended fair value | 340×280 |
| `docs/widgets/options-flow.html` | Call/put volume, P/C ratios | 380×310 |
| `docs/widgets/institutional.html` | TickerTrace ETF holdings | 380×320 |
| `docs/widgets/index.html` | 🔍 Stock lookup page (search + full-hud inline) | N/A |

**Data flow:** Dossier pipeline → `docs/ticker/TSLA/latest.json` → widget fetches JSON → renders

**Color system (3-color):**

- `--green (#00ff41)` = bullish, up, undervalued, buying
- `--red (#f0b400)` = bearish, down, caution (gold/amber — NOT red)
- `--danger (#e53935)` = OVERVALUED, overbought RSI >70, puts bar, institutional selling

**Embedding anywhere:**

```html
<iframe src="https://mphinance.github.io/mphinance/widgets/full-hud.html?t=TSLA" width="700" height="760" frameborder="0" style="border-radius:6px;overflow:hidden;"></iframe>
```

**Only works for watchlist tickers** — data generated by `dossier/generate.py`. Unknown tickers show a friendly error.

**Landing page integration:** `landing/index.html` has an Alpha.HUD section with search + quick picks + iframe pointing to GH Pages. Reports page (`docs/index.html`) has a nav bar linking to widgets.
