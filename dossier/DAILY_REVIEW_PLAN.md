...
- **Next:** (a) wire `universe_scan` into `generate.py` as an early stage feeding the deep screeners (augment/replace the top-200-by-mcap funnel); (b) add the multi-screen union; (c) then triangle / flow / confluence. NOTE: built on the `article/amzn-cpi-referee` branch (uncommitted) — sort branch hygiene before committing dossier work.
