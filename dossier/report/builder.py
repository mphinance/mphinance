...
def build_markdown(
    ...
    if buying:
        ...
        for s in buying[:5]:
            ...
            lines.append(f"- [{s.get('ticker', '')}](https://www.tradingview.com/chart/?symbol={s.get('ticker', '')}) — {s.get('name', '')} ({funds}) Conv: {s.get('conviction', 0)}")
        lines.append("")
    if selling:
    ...
    if scanner_signals:
        ...
        for s in scanner_signals[:15]:
            ...
            lines.append(f"- {emoji} [{s.get('symbol', '')}](https://www.tradingview.com/chart/?symbol={s.get('symbol', '')}) — {s.get('strategy', '')} | {rationale} | Score: {s.get('score', 0)}")
        lines.append("")
    ...
    if technical_setups:
        ...
        for s in technical_setups:
            ...
            lines.append(f"- [{s.get('ticker', '')}](https://www.tradingview.com/chart/?symbol={s.get('ticker', '')}) — {status} | EMA Stack: {stack}")
        lines.append("")
    ...
    if dossiers:
        ...
        for d in dossiers:
            ...
            lines.append(f"### [{d.get('ticker', '')}](https://www.tradingview.com/chart/?symbol={d.get('ticker', '')}) — {d.get('name', '')}")
