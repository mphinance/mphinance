            ...
            for w in sorted(tickers, key=lambda x: x["ticker"]):
                index_html += f"""                        <div class="report-link bg-black/40 border border-gray-800 rounded px-4 py-3 flex items-center justify-between">
                            ...
                            <div class="flex gap-2">
                                <a href="https://www.tradingview.com/chart/?symbol={w['ticker']}" target="_blank" class="text-[9px] text-gray-400 border border-gray-700 px-1.5 py-0.5 rounded hover:text-neon-blue hover:border-neon-blue/30 transition-colors">TV</a>
...
def run_pipeline(date: str, dry_run: bool = False, generate_pdf: bool = True):
    ...
    # Finviz screens DISABLED — backtest showed 23-30% WR across all 5 screens.
    # Negative avg returns at every horizon. Adds noise, not signal.
    # File retained at strategies/finviz_screens.py for future reference.
    print("\n[2a/16] FINVIZ SCREENS — DISABLED (backtest: 23-30% WR, negative returns)")
    # try:
