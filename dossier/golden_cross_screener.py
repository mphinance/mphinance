#!/usr/bin/env python3
"""
🌟 Golden Cross Alert Screener — Fresh 50/200 MA Crossovers

Finds stocks where the 50-day SMA has crossed above the 200-day SMA
(Golden Cross) within the last 15 trading days — the freshest, most
actionable form of the world's most-watched trend-change signal.

The Golden Cross is not a secret. What IS valuable is catching it the week
it happens: before the headlines, before the RSI gets extended, before every
algo on Wall Street piles in. This screener hunts the freshest crossovers so
Michael has actual edge rather than confirmed-by-CNBC lag.

Different from every other screener in the dossier:
  - SMA200 Reclaim: PRICE crossing above SMA200 (one line vs price)
  - NR7 / TAO: coil / pullback setups, not trend-change signals
  - THIS: SMA50 crossing above SMA200 (two moving averages) — the
    institutional trend-change signal, caught fresh (≤15 days old)

Funnel architecture (3-stage, same as sma200 / nr7 / rvol):
    Stage 1 → TradingView bulk API: SMA50 > SMA200 (cross confirmed),
               not yet extended (SMA50 ≤ SMA200 * 1.12), RSI 35–72
...
Scoring (0–100):
    ...
    Trend alignment    (30 pts) — price above EMA20 + SMA50 well above SMA200
    RSI health         (20 pts) — 48–65 ideal; extremes penalised
    Volume expansion   (10 pts) — volume rising around the crossover date

Grades: A+ (80+) · A (65–79) · B (50–64) · C (35–49) · D (<35)

Usage:
    python -m dossier.golden_cross_screener               # whole market
    python -m dossier.golden_cross_screener --tickers NVDA,AAPL
    python -m dossier.golden_cross_screener --watchlist
    python -m dossier.golden_cross_screener --top 20
    python -m dossier.golden_cross_screener --json
    python -m dossier.golden_cross_screener --quiet       # A+/A only
    python -m dossier.golden_cross_screener --no-save     # skip JSON write

Output: docs/api/golden-cross.json (served via GitHub Pages)

© mphinance + Sam the Quant Ghost
"The Golden Cross is late by definition. That's why you want it fresh." — Sam
"""
...
def _tv_fetch_golden_cross_candidates() -> list[dict]:
    """
    One POST to TradingView → pre-filtered golden-cross zone candidates.

    Server-side: SMA50 currently above SMA200 (cross confirmed), not yet
    extended past 12% gap (stale crosses filtered later by yfinance), RSI
    in a healthy non-crashed range.
    """
    payload = {
        "filter": [
            ...
            # Golden cross territory: SMA50 above SMA200
            {"left": "SMA50", "operation": "greater", "right": "SMA200"},
            # Not stale — SMA50 within 12% of SMA200 (fresh cross zone)
            {"left": "SMA50", "operation": "less", "right": "SMA200*1.12"},
            # Healthy RSI — not crashed, not parabolic
            {"left": "RSI", "operation": "in_range", "right": [35, 72]},
        ],
        "options": {"lang": "en"},
    ...
    data = safe_json(resp, "golden-cross TV scan")
...
def _funnel_filter(stocks: list[dict], verbose: bool = True) -> list[dict]:
    ...
    total = len(stocks)
    if verbose:
        print(f"\n  ┌─ GC FUNNEL: {total} stocks from TradingView")
    ...
    # SMA50/SMA200 gap ≤ 12% — stale crosses have wider gaps
    prev = len(survivors)
    survivors = [s for s in survivors if (
        s["sma_50"] <= s["sma_200"] * 1.12
    )]
    if verbose:
...
def detect_golden_cross(sma50: "pd.Series", sma200: "pd.Series",
                        lookback: int = _CROSS_LOOKBACK) -> "int | None":
    """
    Scan backward through aligned SMA series for a golden cross.
...
def _trend_score(price: float, ema20: "float | None",
    ...
    """
    Trend alignment after the golden cross (30 pts max).

    Full marks require price above EMA20 and SMA50 comfortably above SMA200.
    """
    pts = 0
    # Price above 20-day EMA (near-term momentum on their side)
    if ema20 and price > ema20:
        pts += 15
    # SMA50 relative advantage over SMA200 (gap quality)
    if sma200 > 0:
...
def _rsi_score(rsi: float) -> int:
    """RSI health for a post-golden-cross setup (20 pts max)."""
    if 48 <= rsi <= 65:  return 20   # trending zone — ideal
    if 40 <= rsi < 48 or 65 < rsi <= 72:  return 13
    if 35 <= rsi < 40 or 72 < rsi <= 78:  return 6
    return 2
...
def score_golden_cross(ticker: str, tv_data: "dict | None" = None) -> "dict | None":
    """
    Deep scan a single ticker for a fresh golden cross setup.

    Returns a result dict on success, None if data is insufficient or no
    50/200 SMA crossover is detected within the last 15 trading days.
    """
    try:
        ...
        # Must currently have SMA50 above SMA200 (golden cross confirmed)
        if current_sma50 <= current_sma200:
            return None

        # SMA gap > 15% = cross is months old, not fresh
        gap_pct = (current_sma50 - current_sma200) / current_sma200 * 100
        if gap_pct > 15.0:
            return None

        # ── Detect crossover ──────────────────────────────────────
        days_since = detect_golden_cross(sma50, sma200, lookback=_CROSS_LOOKBACK)
        ...
        if pd.isna(rsi_val) or not (30 <= rsi_val <= 85):
            return None
...
def _save_api_output(results: list[dict]) -> None:
    """Write machine-readable JSON to docs/api/golden-cross.json."""
    out_dir = PROJECT_ROOT / "docs" / "api"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "golden-cross.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Golden Cross Alert Screener")
    ...
    if args.tickers:
        ...
        print(f"\n🌟  Golden Cross Screener — {len(tickers)} tickers\n")
        tv_map: dict[str, dict] = {}
        candidates = [{"ticker": t, "name": t} for t in tickers]
    elif args.watchlist:
        tickers = CORE_WATCHLIST
        print(f"\n🌟  Golden Cross Screener — watchlist ({len(tickers)} tickers)\n")
        tv_map = {}
        candidates = [{"ticker": t, "name": t} for t in tickers]
    else:
        print("\n🌟  Golden Cross Screener — whole US equity market\n")
        print("  ⚡ Stage 1: TradingView bulk scan...")
        raw = _tv_fetch_golden_cross_candidates()
        print(f"     → {len(raw)} stocks in golden-cross zone\n")
    ...
    print(f"  🧪 Stage 2: Deep scanning {len(candidates)} candidates for fresh crossovers...")
    results = []
    for i, c in enumerate(candidates):
        ticker = c["ticker"]
        r = score_golden_cross(ticker, tv_data=tv_map.get(ticker, c))
        if r:
            results.append(r)
        if (i + 1) % 25 == 0:
            print(f"     {i + 1}/{len(candidates)} scanned — {len(results)} golden crosses found")
        time.sleep(0.05)
    ...
    print(f"\n{'═' * 100}")
    print(f"  🌟   GOLDEN CROSS ALERT — {len(results)} fresh crossovers (≤{_CROSS_LOOKBACK} days old)")
    ...
    print(f"\n{'─' * 100}")
    print(f"  Legend: Gap = (SMA50 − SMA200) / SMA200 %  |  Cross = days since SMA50 crossed SMA200\n")
