"""
Market Internals feed — combines six existing "breadth-style" daily signals
(breadth_index, factor_leaderboard, score_dispersion, sector_leadership,
quality_breadth, extension_index) into one time-series snapshot for a
dashboard page.
...
dossier/generate.py Stages 10a-10a6) — but nothing reads them back. Today
...
def build_internals(data_dir, window: int = DEFAULT_WINDOW) -> dict:
    """
    Aggregate the six breadth-style history files under `data_dir`
    ...
    "latest" for that metric, same never-raise contract as the six
